...
def main_display_initial_ai_message():
    """
    AIメッセージの初期表示
    """
    with st.chat_message("assistant"):
        ...
        st.warning("⚠️ 具体的に入力したほうが期待通りの回答が得やすいです。")
def side_display_initial_ai_message():
    """
    AIメッセージの初期表示
    """
    # 「社内文書検索」の機能説明
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']}（ページNo{message['main_page_number']}）", icon=icon)
                        else:
...
def display_sidebar():
    ...
    with st.sidebar:
        ...
        # 【「社内文書検索」を選択した場合】
        side_display_initial_ai_message()
